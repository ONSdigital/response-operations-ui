...
def test_no_edit_permission(app, ce_details, expected_ce_context_no_permission):
    ...
    with app.test_request_context():
        context = build_ce_context(ce_details, False, False)
...
def test_has_edit_permission(app, ce_details):
...
def test_exercise_locked(app, ce_details):
...
def test_event_value_dicts(app, ce_details_event_value_dicts):
    # Given a user has edit permission and the event value is a dict
    # When build_ce_context is called
    with app.test_request_context():
        context = build_ce_context(ce_details_event_value_dicts, True, False)
...
def test_event_reminders(app, ce_details_dynamic_event):
    # Given there are 3 reminder events
    # When build_ce_context is called
    with app.test_request_context():
        context = build_ce_context(ce_details_dynamic_event, True, False)
...
def test_event_middle_reminder_deleted(app, ce_details_dynamic_event_deleted):
    # Given are 3 reminder events where one has been deleted (reminder 2 deleted, leaving reminder 1 and 3)
    # When build_ce_context is called
    with app.test_request_context():
        context = build_ce_context(ce_details_dynamic_event_deleted, True, True)
...
def test_locked_event_in_the_past(app, ce_details_event_in_the_past):
    # Given the exercise is locked and an event is in the past
    # When build_ce_context is called
    with app.test_request_context():
        context = build_ce_context(ce_details_event_in_the_past, True, True)

    # Then there is no hyperlink to add/edit
    assert "hyperlink" not in context["action_dates"]["go_live"]


def test_not_locked_event_in_the_past(app, ce_details_event_in_the_past):
    # Given the exercise is not locked and an event is in the past
    # When build_ce_context is called
    with app.test_request_context():
        context = build_ce_context(ce_details_event_in_the_past, True, False)
