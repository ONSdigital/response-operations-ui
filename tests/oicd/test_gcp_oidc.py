...
from response_operations_ui.oidc.gcp_oidc import TTL, OIDCCredentialsServiceGCP
...
@patch("response_operations_ui.oidc.gcp_oidc.Request")
@patch("response_operations_ui.oidc.gcp_oidc.fetch_id_token_credentials")
def test_get_credentials(mock_token_fetch, mock_request, oidc_credentials_service):
    ...
    """
    oidc_credentials_service.get_credentials(iap_client_id=TEST_OAUTH2_CLIENT_ID)
    mock_token_fetch.assert_called_once_with(audience=TEST_OAUTH2_CLIENT_ID, request=mock_request())
    mock_token_fetch.return_value.refresh.assert_called_once_with(mock_request())


@patch("response_operations_ui.oidc.gcp_oidc.Request", Mock)
@patch("response_operations_ui.oidc.gcp_oidc.fetch_id_token_credentials", Mock(side_effect=RefreshError))
...
def test_get_credentials_with_retry(oidc_credentials_service):
    ...
    with patch("response_operations_ui.oidc.gcp_oidc.Request", Mock(return_value=tracked_request)):
        credentials = oidc_credentials_service.get_credentials(iap_client_id=TEST_OAUTH2_CLIENT_ID)
...
def test_get_credentials_transport_failure(oidc_credentials_service):
    ...
    with patch("response_operations_ui.oidc.gcp_oidc.Request", Mock(return_value=failing_request)):
        with pytest.raises(RefreshError):
            oidc_credentials_service.get_credentials(iap_client_id=TEST_OAUTH2_CLIENT_ID)
