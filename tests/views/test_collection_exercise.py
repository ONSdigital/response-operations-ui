...
class TestCollectionExercise(ViewTestCase):
    ...
    @patch("response_operations_ui.common.filters.build_eq_ci_selectors")
    @patch("response_operations_ui.views.collection_exercise.build_collection_exercise_details")
    def test_failed_add_eq_collection_instrument(
        self, mock_request, mock_details, mock_collective_cis, mock_ci_selector, mock_get_cis_and_cir_version
    ):
        sign_in_with_permission(self, mock_request, user_permission_surveys_edit_json)
        post_data = {"checkbox-answer": [collection_instrument_id], "ce_id": collection_exercise_id, "select-eq-ci": ""}
        mock_request.post(
    ...
    @requests_mock.mock()
    @patch("response_operations_ui.views.collection_exercise.collection_instrument_controllers.get_cis_and_cir_version")
    def test_eq_view_sample_ci_page_survey_permission_with_cir(self, mock_request, mock_get_cis_and_cir_version):
        sign_in_with_permission(self, mock_request, user_permission_surveys_edit_json)
        mock_get_cis_and_cir_version.return_value = [{"form_type": "0001", "ci_version": None}]
        mock_request.get(url_get_survey_by_short_name, json=self.eq_survey_dates)
        mock_request.get(url_ces_by_survey, json=self.collection_exercises)
    ...
    @requests_mock.mock()
    @patch("response_operations_ui.views.collection_exercise.collection_instrument_controllers.get_cis_and_cir_version")
    def test_linked_ci_eq_view_sample_ci_page_survey_permission(self, mock_request, mock_get_cis_and_cir_version):
        sign_in_with_permission(self, mock_request, user_permission_surveys_edit_json)
        mock_get_cis_and_cir_version.return_value = [{"form_type": "0001", "ci_version": None}]
        mock_request.get(url_get_survey_by_short_name, json=self.eq_survey_dates)
        mock_request.get(url_ces_by_survey, json=self.collection_exercises)
    ...
    def test_linked_ci_eq_view_sample_ci_page_no_survey_permission(
        self, mock_request, mock_details, mock_get_cis_and_cir_version
    ):
        mock_request.get(url_get_survey_by_short_name, json=self.eq_survey_dates)
        mock_request.get(url_ces_by_survey, json=self.collection_exercises)
        mock_request.get(url_ce_by_id, json=collection_exercise_eq_both_ref_date["collection_exercise"])
    ...
    @requests_mock.mock()
    @patch("response_operations_ui.views.collection_exercise.collection_instrument_controllers.get_cis_and_cir_version")
    def test_add_collection_instrument_success(self, mock_request, mock_get_cis_and_cir_version):
        sign_in_with_permission(self, mock_request, user_permission_surveys_edit_json)
    ...
    @requests_mock.mock()
    @patch("response_operations_ui.views.collection_exercise.collection_instrument_controllers.get_cis_and_cir_version")
    def test_add_collection_instrument_duplicate(self, mock_request, mock_get_cis_and_cir_version):
        sign_in_with_permission(self, mock_request, user_permission_surveys_edit_json)
        post_data = {"formtype": "0001", "add-eq-ci": ""}
        mock_get_cis_and_cir_version.return_value = [{"form_type": "0001", "ci_version": None}]
        mock_request.get(url_survey_shortname, json=self.single_survey_eq)
