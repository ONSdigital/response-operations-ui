...
eq_url_mapping = {"preprod": "https://eq.onsdigital.uk/flush", "prod": "https://eq.ons.gov.uk/flush"}


def parse_args():
    parser = argparse.ArgumentParser(
        description="Flush partially completed responses from EQ. In order to run, a file"
...
def call_flush_endpoint(eq_url, token):
    ...
    try:
        ...
        print(e)
        print(
            "Flush failed.  A 404 might indicate that the payload was correct, but EQ doesn't have a record of "
            "the submission"
        )
