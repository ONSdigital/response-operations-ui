...
def get_collection_exercise(short_name, period):
    ...
    logger.debug(
        "Successfully retrieved collection exercise details", short_name=short_name, period=period
    )
...
def update_event(short_name, period, tag, timestamp):
    ...
    logger.debug("Successfully updated event date", short_name=short_name, period=period, tag=tag)
    return True
...
def get_collection_exercise_by_id(collection_exercise_id):
    ...
    try:
        ...
        log_level(
            "Failed to retrieve collection exercise", collection_exercise_id=collection_exercise_id
        )
        raise ApiError(response)

    logger.debug(
        "Successfully retrieved collection exercise", collection_exercise_id=collection_exercise_id
    )
    return response.json()
...
def create_collection_exercise(survey_id, survey_name, user_description, period):
    logger.debug(
        "Creating a new collection exercise for", survey_id=survey_id, survey_name=survey_name
    )
    ...
    logger.debug(
        "Successfully created collection exercise for", survey_id=survey_id, survey_name=survey_name
    )
