...
def get_cir_service_status():
    response = requests.get(app.config["CIR_API_URL"] + "/status")
    ...
    fetch_and_apply_oidc_credentials(session=requests.session(), client_id=app.config["CIR_OAUTH2_CLIENT_ID"])
    return {"status": 200}
